...
class DCGAN(object):
    def __init__(self, sess, image_size=96, is_crop=True,
                 batch_size=64, sample_size = 64, image_shape=[64, 96, 3],
                 y_dim=None, z_dim=100, gf_dim= 80, df_dim=80,
    ...
    def generator(self, z, y=None):
        if not self.y_dim:
            ...
            self.z_, self.h0_w, self.h0_b = linear(z, self.gf_dim*8*4*6, 'g_h0_lin', with_w=True)

            self.h0 = tf.reshape(self.z_, [-1, 4, 6, self.gf_dim * 8])
            h0 = tf.nn.relu(self.g_bn0(self.h0))

            self.h1, self.h1_w, self.h1_b = deconv2d(h0, 
                [self.batch_size, 8, 12, self.gf_dim*4], name='g_h1', with_w=True)
            h1 = tf.nn.relu(self.g_bn1(self.h1))

            h2, self.h2_w, self.h2_b = deconv2d(h1,
                [self.batch_size, 16, 24, self.gf_dim*2], name='g_h2', with_w=True)
            h2 = tf.nn.relu(self.g_bn2(h2))

            h3, self.h3_w, self.h3_b = deconv2d(h2,
                [self.batch_size, 32, 48, self.gf_dim*1], name='g_h3', with_w=True)
            h3 = tf.nn.relu(self.g_bn3(h3))

            h4, self.h4_w, self.h4_b = deconv2d(h3,
                [self.batch_size, 64, 96, 3], name='g_h4', with_w=True)
    ...
    def sampler(self, z, y=None):
        ...
        if not self.y_dim:
            # project `z` and reshape
            h0 = tf.reshape(linear(z, self.gf_dim*8*4*6, 'g_h0_lin'),
                            [-1, 4, 6, self.gf_dim * 8])
            h0 = tf.nn.relu(self.g_bn0(h0, train=False))

            h1 = deconv2d(h0, [self.batch_size, 8, 12, self.gf_dim*4], name='g_h1')
            h1 = tf.nn.relu(self.g_bn1(h1, train=False))

            h2 = deconv2d(h1, [self.batch_size, 16, 24, self.gf_dim*2], name='g_h2')
            h2 = tf.nn.relu(self.g_bn2(h2, train=False))

            h3 = deconv2d(h2, [self.batch_size, 32, 48, self.gf_dim*1], name='g_h3')
            h3 = tf.nn.relu(self.g_bn3(h3, train=False))

            h4 = deconv2d(h3, [self.batch_size, 64, 96, 3], name='g_h4')
